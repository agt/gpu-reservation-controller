...
| `CANCEL_VACATED_RESERVATIONS` | no | `true` | Cancel a reservation once its workload terminates and no pods remain — a JIT lease whose only pod exits (even mid-window), or an overstay booking whose last pod has gone. A booking still inside its window is never cancelled. Set to `false` to disable |
