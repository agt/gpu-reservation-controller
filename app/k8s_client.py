...
async def emit_lease_renewed_event(
    pod,
    pod_name: str,
    namespace: str,
    reservation_id: int,
    ...
    """Create a Kubernetes Event linked to *pod* with reason='LeaseRenewed'.

    Emitted when an on-demand lease nearing expiry is renewed (chained) in place,
    so the pod keeps running under the same reservation id with a later
    guarantee.
    """
    until_str = guaranteed_until.strftime("%Y-%m-%dT%H:%M:%SZ")
    await _emit_pod_event(
        ...
        pod_name,
        namespace,
        name_prefix="gpu-renew-",
        reason="LeaseRenewed",
        action="RenewLease",
        message=(
            f"On-demand lease #{reservation_id} renewed; GPU access now "
            f"guaranteed until {until_str}."
        ),
    )
    log.info(
        "Emitted LeaseRenewed event for pod %s/%s (reservation=#%d, until=%s)",
        namespace,
        pod_name,
        reservation_id,
        until_str,
...
async def emit_renewal_denied_event(
    pod,
    pod_name: str,
    namespace: str,
    reservation_id: int,
    ...
    """Create a Kubernetes Event linked to *pod* with reason='RenewalDenied'.

    Emitted once when an on-demand lease nearing expiry could not be renewed for
    lack of capacity/budget (a 409 from the app).  The lease runs out its
    remaining time, so this is the user's cue to checkpoint before eviction.
    """
    ...
    await _emit_pod_event(
        ...
        pod_name,
        namespace,
        name_prefix="gpu-renewdenied-",
        reason="RenewalDenied",
        action="RenewLease",
        message=(
            f"On-demand lease #{reservation_id} could not be renewed (no GPU "
            f"capacity available); the lease ends at {ends_str}. Checkpoint your "
            f"work before then to avoid losing progress."
        ),
    )
    log.info(
        "Emitted RenewalDenied event for pod %s/%s (reservation=#%d, ends=%s)",
        namespace,
        pod_name,
        reservation_id,
        ends_str,
