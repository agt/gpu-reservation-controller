"""Unit tests for overstay-pod adoption (re-linking a running pod to a new booking).

Covers the pure planning surface added for the "user re-books while their pod is
still running" scenario:

  - ``ControllerState.find_open_booking_for`` — open-booking selection
  - ``ControllerState.plan_overstay_adoptions`` — which overstay pods adopt where
...
All pure / no I/O — the async orchestrator (``main._adopt_overstay_pods``) is not
exercised here.
"""
...
# ---------------------------------------------------------------------------
# plan_overstay_adoptions
# ---------------------------------------------------------------------------


class TestPlanOverstayAdoptions:
    def test_non_abutting_booking_adopts_overstay_pod(self):
        ...
        plan = state.plan_overstay_adoptions([pod], NOW)
        assert len(plan) == 1
    ...
    def test_pod_within_guarantee_left_alone(self):
        ...
        # to 17:00, so it is not overstay and is not adopted.
        old = _old_booking(100)
        abutting = _open_booking(200, start_utc=THREE_PM)  # abuts old's 15:00 end
        state = _state(old, abutting)
        pod = _view("p1", reservation_id=100, reserved_path=True)
        assert state.plan_overstay_adoptions([pod], NOW) == []
    ...
    def test_budget_respected_across_pods(self):
        ...
        plan = state.plan_overstay_adoptions([p1, p2], NOW)
