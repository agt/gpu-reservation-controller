...
class TestGetPodMinRuntimeSeconds:
    def test_valid_annotation(self):
        pod = _pod(annotations={"horai/minimum-runtime-seconds": "3600"})
        assert get_pod_min_runtime_seconds(pod) == 3600

    def test_minimum_positive_value(self):
        pod = _pod(annotations={"horai/minimum-runtime-seconds": "1"})
    ...
    def test_non_integer_value_returns_none(self):
        pod = _pod(annotations={"horai/minimum-runtime-seconds": "abc"})
        assert get_pod_min_runtime_seconds(pod) is None

    def test_zero_returns_none(self):
        pod = _pod(annotations={"horai/minimum-runtime-seconds": "0"})
        assert get_pod_min_runtime_seconds(pod) is None

    def test_negative_returns_none(self):
        pod = _pod(annotations={"horai/minimum-runtime-seconds": "-100"})
        assert get_pod_min_runtime_seconds(pod) is None
